...
public class MainMenuScreen extends Screen {
    ...
    private float getLogoScale(long currentTime) {
        return MathHelper.lerp(easeOutBack(getEntranceProgress(currentTime, ENTRANCE_LOGO_DELAY)), 0.3f, 1f);
    }

    private float getLogoAlpha(long currentTime) {
        float t = easeOutBack(getEntranceProgress(currentTime, ENTRANCE_LOGO_DELAY));
        return t < 0.01f ? 0f : t;
    }

    private float getGreetingEntrance(long currentTime) {
        return easeOutCubic(getEntranceProgress(currentTime, ENTRANCE_GREETING_DELAY));
    }

    private float getGreetingSlideY(long currentTime) {
        return (1f - easeOutCubic(getEntranceProgress(currentTime, ENTRANCE_GREETING_DELAY))) * 15f;
    }

    private float getSubtextEntrance(long currentTime) {
        return easeOutCubic(getEntranceProgress(currentTime, ENTRANCE_SUBTEXT_DELAY));
    }

    private float getSubtextSlideY(long currentTime) {
        return (1f - easeOutCubic(getEntranceProgress(currentTime, ENTRANCE_SUBTEXT_DELAY))) * 12f;
    }

    private float getButtonEntrance(int index, long currentTime) {
        return easeOutCubic(getEntranceProgress(currentTime, ENTRANCE_BUTTON_START_DELAY + index * ENTRANCE_BUTTON_STAGGER));
    }

    private float getButtonSlideY(int index, long currentTime) {
        return (1f - getButtonEntrance(index, currentTime)) * 20f;
    }

    private float getFooterEntrance(long currentTime) {
        return easeOutCubic(getEntranceProgress(currentTime, ENTRANCE_FOOTER_DELAY));
    }
    ...
    private void renderMainMenu(int screenWidth, int screenHeight, float mouseX, float mouseY, long currentTime) {
        ...
        float logoAlpha = getLogoAlpha(currentTime);
        float logoScale = getLogoScale(currentTime);
        ...
        float greetingAlpha = getGreetingEntrance(currentTime);
        float greetingSlideY = getGreetingSlideY(currentTime);
        ...
        float subtextAlpha = getSubtextEntrance(currentTime);
        float subtextSlideY = getSubtextSlideY(currentTime);
        ...
        for (int i = 0; i < 2; i++) {
            float btnAlpha = getButtonEntrance(i, currentTime);
            float btnSlide = getButtonSlideY(i, currentTime);
            if (btnAlpha > 0.01f) {
        ...
        float btn2Alpha = getButtonEntrance(2, currentTime);
        float btn2Slide = getButtonSlideY(2, currentTime);
        if (btn2Alpha > 0.01f) {
        ...
        for (int i = 0; i < 3; i++) {
            float btnAlpha = getButtonEntrance(i + 3, currentTime);
            float btnSlide = getButtonSlideY(i + 3, currentTime);
            if (btnAlpha > 0.01f) {
        ...
        float footerAlpha = getFooterEntrance(currentTime);
