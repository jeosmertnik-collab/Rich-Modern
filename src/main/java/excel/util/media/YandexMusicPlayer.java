...
public class YandexMusicPlayer {

    private final HttpClient httpClient = HttpClient.newHttpClient();
    private YandexMusicApi ymApi;
    private List<YandexTrack> playlist = new ArrayList<>();
    private int currentTrackIndex = -1;
    private boolean playing = false;
    private boolean paused = false;
    private boolean ymReady = false;
    ...
    public void init() {
        ...
        if (token.isEmpty()) {
            currentTrackName = "Yandex Music: no token";
            return;
        }
        ymApi = new YandexMusicApi(token);
        new Thread(() -> {
            try {
                if (!ymApi.init()) {
                    currentTrackName = "Yandex Music: auth failed";
                    return;
                }
                List<YandexTrack> tracks = ymApi.fetchTracks();
                if (tracks != null && !tracks.isEmpty()) {
                    ...
                    currentTrackName = "Yandex Music: no tracks";
                }
            } catch (Exception e) {
                currentTrackName = "YM error: " + e.getMessage();
            }
        }, "YM-Init").start();
    }
    ...
    private String readToken() {
        String[] paths = {
            "Excel/configs/ym_token.txt",
            "../Excel/configs/ym_token.txt",
            "Rich/configs/ym_token.txt",
            "../Rich/configs/ym_token.txt",
        };
        for (String p : paths) {
            try {
    ...
    public void playCurrent() {
        if (playlist.isEmpty() || !ymReady) return;
        if (currentTrackIndex < 0 || currentTrackIndex >= playlist.size()) currentTrackIndex = 0;
        stopPlayback();
        updateTrackName();

        YandexTrack track = playlist.get(currentTrackIndex);
        if (track.downloadUrl == null || track.downloadUrl.isEmpty()) {
            if (!ymApi.resolveDownloadUrl(track)) {
                currentTrackName = "YM: cannot get track URL";
                return;
            }
        }
        stopRequested = false;
        ...
        playbackThread = new Thread(() -> playTrack(track), "YM-Playback");
        playbackThread.setDaemon(true);
        playbackThread.start();
    }

    private void playTrack(YandexTrack track) {
        try {
            String trackUrl = track.downloadUrl;
    ...
    private void updateTrackName() {
        if (playlist.isEmpty() || currentTrackIndex < 0 || currentTrackIndex >= playlist.size()) {
            currentTrackName = ymReady ? "No tracks" : "Yandex Music: enter token in launcher";
            return;
        }
        YandexTrack t = playlist.get(currentTrackIndex);
    ...
    public boolean isYmReady() { return ymReady; }
    public List<YandexTrack> getPlaylist() { return playlist; }
